...
namespace xF {
	...
	struct xSkinWeights {
		...
		Matrix4D				MatrixFinal;
		Matrix4D				MatrixOffset;
		Matrix4D				*MatrixCombined;
		Matrix4D				*MatrixCombinedAnimation;
		std::string				NodeName;
	};
...
//			delete[] pData;
	...
	struct xMeshGeometry {
		...
		std::vector<Vector2>	 TexCoordinates[4];

		std::vector<xWORD>		 Triangles;
		xSkinInfo				 Info;
		xMaterialList			 MaterialList;

		Matrix4D				 RelativeMatrix;
		std::string				 Name;
		xDWORD					 NumVertices;
		xDWORD					 NumTriangles;
		xDWORD					 NumIndices;
