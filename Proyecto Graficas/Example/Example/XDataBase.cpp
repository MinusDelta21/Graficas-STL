...
namespace xF {
	...
	void XDataBase::ProcessMeshBlock(std::string actual) {
		...
		ptr->Positions = std::vector<Vector3>(ptr->NumVertices);
#endif
...
#if USE_VECTOR_RESERVE_AND_PUSH
			ptr->Positions.push_back(Vector3(x, y, z));
#elif USE_VECTOR_ARRAY_MODE
			ptr->Positions[i].x = x;
			ptr->Positions[i].y = y;
			ptr->Positions[i].z = z;
...
{		TimeEvent t("Indices Block");
	...
	void  XDataBase::ProcessSkinWeights(xF::xMeshGeometry* pGeometry) {
		...
		for (int i = 0; i < 16; i++) {
			m_ActualStream >> pSkin->MatrixOffset.v[i] >> c_temp;
		}
...
#elif USE_VECTOR_ARRAY_MODE
		pGeometry->Normals = std::vector<Vector3>(NumVertices);
#endif
		float x = 0.0f, y = 0.0f, z = 0.0f;
		for (std_uint i = 0; i < NumVertices; i++) {
			m_ActualStream >> x >> c_temp >> y >> c_temp >> z >> c_temp >> c_temp;
#if USE_VECTOR_RESERVE_AND_PUSH
			pGeometry->Normals.push_back(Vector3(x, y, z));
#elif USE_VECTOR_ARRAY_MODE
...
#elif USE_VECTOR_ARRAY_MODE
		pGeometry->TexCoordinates[pGeometry->NumChannelsTexCoords] = std::vector<Vector2>(NumTexcoords);
#endif
		for (std_uint i = 0; i < pGeometry->Normals.size(); i++) {
			m_ActualStream >> x >> c_temp >> y >> c_temp >> c_temp;
#if USE_VECTOR_RESERVE_AND_PUSH
			pGeometry->TexCoordinates[pGeometry->NumChannelsTexCoords].push_back(Vector2(x, y));
#elif USE_VECTOR_ARRAY_MODE
		...
		for (unsigned int i = 0; i < NumElements; i++) {
			if (Data.Elements[i].Usage == xF::STDDECLUSAGE_TANGENT) {
...
#elif USE_VECTOR_ARRAY_MODE
				pGeometry->Tangents = std::vector<Vector3>(NumVertices);
#endif
				int Offset = 3;
				for (unsigned int j = 0; j < NumVertices; j++) {
					...
					pGeometry->Tangents.push_back(Vector3(x, y, z));
#elif USE_VECTOR_ARRAY_MODE
...
#elif USE_VECTOR_ARRAY_MODE
				pGeometry->Binormals = std::vector<Vector3>(NumVertices);
#endif
				int Offset = 0;
				for (unsigned int j = 0; j < NumVertices; j++) {
#if USE_VECTOR_RESERVE_AND_PUSH
					...
					y = *(float*)(&Data.Values[Offset++]);
					z = *(float*)(&Data.Values[Offset++]);
					pGeometry->Binormals.push_back(Vector3(x, y, z));
#elif USE_VECTOR_ARRAY_MODE
...
#elif USE_VECTOR_ARRAY_MODE
				pGeometry->TexCoordinates[pGeometry->NumChannelsTexCoords] = std::vector<Vector2>(NumVertices);
#endif
				int Offset = 0;
				for (unsigned int j = 0; j < NumVertices; j++) {
#if USE_VECTOR_RESERVE_AND_PUSH
...
#elif USE_VECTOR_ARRAY_MODE
				pGeometry->VertexColors = std::vector<Vector3>(NumVertices);
#endif
				int Offset = 0;
				for (unsigned int j = 0; j < NumVertices; j++) {
#if USE_VECTOR_RESERVE_AND_PUSH
					...
					pGeometry->VertexColors.push_back(Vector3(x, y, z, w));
#elif USE_VECTOR_ARRAY_MODE
	...
	void XDataBase::ProcessMatrix(Matrix4D * out) {
#if PROFILE_MATRICES
